interface ListErrors {
  ...
  emails: () => string,
  dateMajor: () => string,
  maxCurrency: ({requiredValue}: any) => string,
  usernameExists: () => string,
  documentNumberExists: () => string,
  personEmailExists: () => string,
  existRoleName: () => string,
  existGroupName: () => string,
  existSerial: () => string,
  existImei: () => string,
  IP: () => string,
  polCustodio: () => string,
...
export const ListErrors: ListErrors = {
  ...
  maxCurrency: ({
                  requiredValue
                }): string => `El valor no debe ser mayor a <strong>${requiredValue}</strong>`,
  usernameExists: () => 'El nombre de usuario ya existe',
  documentNumberExists: () => 'El numero de documento ya existe',
  personEmailExists: () => 'Esta direccion de correo electronico ya existe',
  existRoleName: () => 'Ya existe un rol con este nombre',
  existSerial: () => 'Ya existe este número de serial',
  existImei: () => 'Ya existe este número de IMEI',
  existGroupName: () => 'Ya existe un grupo con este nombre',
  IP: () => 'Dirección IP no valida',
  polCustodio: () => 'No es posible seleccionar los roles \'pol-custodio-1\' y \'pol-custodio-2\' al mismo tiempo.',
